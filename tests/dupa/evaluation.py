...
from config import fingerprint_classes, filter_dict

def check_combinations(processor, model_pipeline, dataset_name, results):
    accuracy_difference = 0
    train_X, train_y, valid_X, valid_y, test_X, test_y = processor.get_filtered_data(None)

    for fingerprint_name, fingerprint_class in fingerprint_classes.items():
        base_accuracy = model_pipeline.process(train_X, train_y, test_X, test_y, fingerprint_class)
        results.append(
            {
        ...
        for filter_names in combinations(["rule_of_3", "faf4_druglike", "lint"], 2):
            ...
            train_X, train_y, valid_X, valid_y, test_X, test_y = processor.get_filtered_data(filter_fns)

            accuracy = model_pipeline.process(train_X, train_y, test_X, test_y, fingerprint_class)
...
def check_filters(processor, model_pipeline, dataset_name, results):
    ...
    for fingerprint_name, fingerprint_class in fingerprint_classes.items():
        for filter_name, filter_fn in filter_dict.items():
            train_X, train_y, valid_X, valid_y, test_X, test_y = processor.get_filtered_data(filter_fn)
